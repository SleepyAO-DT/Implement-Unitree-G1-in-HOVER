...
class MotionLibG1(MotionLibBase):
    ...
    def __init__(self, motion_file, device, fix_height=FixHeightMode.no_fix, masterfoot_conifg=None, min_length=-1, im_eval=False, multi_thread=True, extend_hand = True, extend_head = False, mjcf_file="resources/robots/g1/g1_29dof_anneal_23dof_without_wrist.xml", sim_timestep = 1/50):
        ...
        for i, data in enumerate(self._motion_data_list):
            if isinstance(data, dict):
                ...
                if 'root_trans_offset' in data:
                    ...
                    # 尝试提取内层数据
                    # 查找包含运动数据的键
                    found_inner_data = False
                    for key, value in data.items():
                        if isinstance(value, dict):
                            # 检查内层是否包含运动数据
                            if 'root_trans_offset' in value:
                                fixed_data_list.append(value)
                                found_inner_data = True
                                break
                            # 检查是否是 {outer_key: {outer_key: actual_data}} 结构
                            elif key in value and isinstance(value[key], dict):
                                if 'root_trans_offset' in value[key]:
                                    fixed_data_list.append(value[key])
                                    found_inner_data = True
                                    break
                    
                    if not found_inner_data:
                        # 没有找到内层数据，保留原样
                        fixed_data_list.append(data)
            else:
                # 不是字典，直接添加
        ...
        # 替换为修复后的数据列表
        self._motion_data_list = fixed_data_list
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1, target_heading = None):
        ...
        # 将 numpy 数组转换为整数列表
        sample_indices = sample_idxes.cpu().numpy()
        # 确保是整数类型
        sample_indices = sample_indices.astype(int)
        
        # 使用列表推导式获取数据，而不是直接用 numpy 数组索引
        motion_data_list = [self._motion_data_list[i] for i in sample_indices]
    ...
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, gender_betas, fix_height, mesh_parsers, masterfoot_config,  target_heading, max_len, queue, pid):
        ...
        for f in range(len(motion_data_list)):
            ...
            # # 处理双层嵌套结构
            # if isinstance(curr_file, dict) and 'root_trans_offset' not in curr_file:
            #     # 检查是否是双层嵌套：{outer_key: {outer_key: actual_data}}
            #     # 或者 {outer_key: {inner_key: actual_data}}
            #     for key, value in curr_file.items():
            #         if isinstance(value, dict):
            #             # 检查内层是否包含所需数据
            #             if 'root_trans_offset' in value:
            #                 curr_file = value
            #                 break
            #             # 如果是完全嵌套：{outer_key: {outer_key: actual_data}}
            #             elif key in value and isinstance(value[key], dict):
            #                 if 'root_trans_offset' in value[key]:
            #                     curr_file = value[key]
            #                     break
