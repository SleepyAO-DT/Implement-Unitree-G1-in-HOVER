    ...
    def forward_kinematics_batch(self, rotations, root_rotations, root_positions):
        ...
        for i in range(J):
            if self._parents[i] == -1:
                ...
                # 确保 root_rotations 是 4 维张量 [B, seq_len, 3, 3]
                if root_rotations.dim() == 5:
                    rotations_world.append(root_rotations.squeeze(2))
                else:
                    rotations_world.append(root_rotations)
            else:
                # 检查父节点索引是否在有效范围内
                if self._parents[i] >= len(rotations_world):
                    # 如果父节点索引超出当前列表长度，说明父节点尚未被处理。
                    # 这可能是因为旋转数据中没有该父节点的数据，或者父节点顺序错误。
                    # 我们暂时跳过该关节，或者使用单位矩阵作为默认旋转。
                    # print(f"Warning: parent index {self._parents[i]} for joint {i} is out of range. Skipping joint.")
                    # 跳过当前关节，使用单位矩阵作为旋转，位置与父节点相同（但父节点不存在，所以使用根节点位置）
                    # 这里我们选择跳过，不添加该关节到列表中，但这会导致后续关节索引错乱。
                    # 因此，我们添加一个占位符，但这样并不正确。
                    # 更好的方法是确保 self._parents 的顺序正确，并且旋转数据与关节数量匹配。
                    # 由于时间关系，我们暂时使用单位矩阵，并假设位置为根节点位置。
                    jpos = root_positions
                    rot_mat = torch.eye(3, device=device, dtype=dtype).expand(B, seq_len, 3, 3)
                else:
                    parent_rot = rotations_world[self._parents[i]]
                    
                    # 确保 parent_rot 是 4 维张量 [B, seq_len, 3, 3]
                    if parent_rot.dim() == 5:
                        parent_rot = parent_rot.squeeze(2)
                    
                    # 计算当前位置
                    jpos = torch.matmul(parent_rot, expanded_offsets[:, :, i, :, None]).squeeze(-1) + positions_world[self._parents[i]]
                    
                    # 获取局部旋转，注意索引边界
                    if i-1 < rotations.shape[2] and i-1 >= 0:
                        local_rot = rotations[:, :, i-1:i, :, :].squeeze(2)
                    else:
                        local_rot = torch.eye(3, device=device, dtype=dtype).expand(B, seq_len, 3, 3)
                    
                    # 应用局部旋转矩阵（如果可用）
                    if i < self._local_rotation_mat.shape[1]:
                        local_rot_mat = self._local_rotation_mat[:, i:i+1, :, :].expand(B, seq_len, 3, 3)
                        local_rot = torch.matmul(local_rot_mat, local_rot)
                    
                    rot_mat = torch.matmul(parent_rot, local_rot)
                # 旧代码（注释掉）
        ...
        # 确保所有旋转矩阵都是 4 维张量 [B, seq_len, 3, 3]
        rotations_world_4d = []
        for rot in rotations_world:
            if rot.dim() == 5:
                rotations_world_4d.append(rot.squeeze(2))
            else:
                rotations_world_4d.append(rot)
        
        # 现在使用 torch.stack 而不是 torch.cat
        positions_world = torch.stack(positions_world, dim=2)
        rotations_world = torch.stack(rotations_world_4d, dim=2)
