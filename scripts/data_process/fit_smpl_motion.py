...
def main(cfg : DictConfig) -> None:
    all_pkls = glob.glob("./humanoidverse/data/AMASS/AMASS_Complete/**/*.npz", recursive=True)
    hardcode_motion_name_idx = len("./humanoidverse/data/AMASS/AMASS_Complete/".split("/"))
    ...
    if len(key_name_to_pkls) == 0:
        raise ValueError(f"No motion files found")
    ...
    for key_name in key_names:
        ...
        if len(current_data) > 0:  # Only save if processing was successful
            os.makedirs(f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/AMASS/singles", exist_ok=True)
            dumped_file = f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/AMASS/singles/{key_name}.pkl"
            logger.info(colored(f"Dumping to {dumped_file}", "green"))
            joblib.dump(current_data, dumped_file)
            all_data[key_name] = current_data
    if all_data:
        total_file = f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/AMASS/amass_all_g1.pkl"
        logger.info(colored(f"Dumping all data to {total_file} (total: {len(all_data)} motions)", "green"))
        joblib.dump(all_data, total_file)
