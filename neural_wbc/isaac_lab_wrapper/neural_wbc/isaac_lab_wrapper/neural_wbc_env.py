...
class NeuralWBCEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: NeuralWBCEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        additional_ids = list(range(self._body_ids[-1]+1,self._body_ids[-1]+len(self.cfg.extend_body_names)+1))
        ...
        # Debug printing cadence (steps). Set to 0 to disable.
        self._debug_print_interval = 1000
    ...
    def _get_observations(self) -> dict:
        ...
        ref_motion_state: ReferenceMotionState = self._ref_motion_mgr.get_state_from_motion_lib_cache(
            ...
            body_names_extend=self._body_names_extend,
        )
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self._debug_print_interval > 0 and self.common_step_counter % self._debug_print_interval == 0:
            mean_reward = reward_sum.mean().item()
            mean_terms = {name: tensor.mean().item() for name, tensor in rewards.items()}
            print(f"[DEBUG] step={self.common_step_counter} mean_reward={mean_reward:.3f} mean_terms={mean_terms}")
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._debug_print_interval > 0 and self.common_step_counter % self._debug_print_interval == 0:
            term_counts = {
                name: int(cond.sum().item()) for name, cond in self._termination_conditions.items()
            }
            print(f"[DEBUG] step={self.common_step_counter} termination_counts={term_counts}")

            # Report which bodies have the largest position mismatch.
            diff = torch.norm(body_state.body_pos_extend - ref_motion_state.body_pos_extend, dim=-1)
            mean_diff = diff.mean(dim=0)
            top_k = mean_diff.numel()
            values, indices = torch.topk(mean_diff, k=top_k)
            top_mismatches = [
                f"{self._body_names_extend[i]}={values[n].item():.3f}"
                for n, i in enumerate(indices.tolist())
            ]
            print(f"[DEBUG] step={self.common_step_counter} top_body_pos_diff={top_mismatches}")
